...
export class SpacesComponent implements OnInit {
  ...
  constructor(private spaceService: SpacesService, private loginService: LoginService,
    private taskService: TasksService, public dialog: MatDialog, private route: Router) { }
  ...
  openDialog() {
    this.dialog.open(AddTaskComponent).afterClosed().subscribe
    ( (data : any) =>{
  ...
  deleteTask(task: any) {
    ...
    this.taskService.deleteTask(taskId).subscribe((data: any) => {
      this.currentSpace.taskList = this.currentSpace.taskList.filter((x: { taskId: any; }) => x.taskId != taskId);
  ...
  drop(event: CdkDragDrop<any[]>) {
    ...
    if (event.previousContainer === event.container) {
      ...
      switch (container) {
        ...
        console.log(temp)
      this.taskService.updateTask(temp).subscribe(
        (res) => {
              console.log(res);
        }
      )
  ...
  updateDueDate(task: any, event: any) {
    ...
    this.taskService.updateTask(task).subscribe(
      (res: any) => console.log(res.taskName + " Updated")
    )
  }
  ...
  specificTask: any = [
    { taskName: "", email: "", taskDescription: "", startDate: "", dueDate: "", edits: "" }
  ];

  displayedColumns: string[] = ['taskName', 'email', 'taskDescription', 'startDate', 'dueDate', 'edits'];
